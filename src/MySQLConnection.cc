...
uint32_t MySQLResult::fields() { return mysql_num_fields(result); }

char **MySQLResult::nextRow() { return mysql_fetch_row(result); }
...
bool MySQLConnection::open() {
  ...
  if (!conn) { LOG(ERROR) << "create MYSQL failed"; }
...
bool MySQLConnection::query(const char *sql, MySQLResult &result) {
  ...
  if (res) result.reset(mysql_store_result(conn));
  return res;
}
...
uint64_t MySQLConnection::affectedRows() { return mysql_affected_rows(conn); }

uint64_t MySQLConnection::getInsertId() { return mysql_insert_id(conn); }
...
string MySQLConnection::getVariable(const char *name) {
  ...
  MySQLResult result;
  if (!query(sql, result) || result.numRows() == 0) { return ""; }
